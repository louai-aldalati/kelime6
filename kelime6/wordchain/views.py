...
def playWordchain(request):
    ...
    if request.method == 'POST':
        ...
        try:
            next_word_obj = Word.objects.get(eng_word__iexact=eng_input)
    ...
    if not game:
        ...
        first = Word.objects.order_by('?').first()
        WordChainEntry.objects.create(
            word_chain_game=game,
            word=first.eng_word,
            position=1
    ...
    candidates = Word.objects.filter(eng_word__istartswith=last_char).order_by('eng_word')

    # بنية الإدخالات مع الترجمة
    entries_with_trans = []
    for e in entries:
        w = Word.objects.get(eng_word=e.word)
